...
#ifdef HIL_MODE
    /* ── HILSYN — handshake step 2: ground station identifies itself ─────
     *   Ground station sends "HILSYN\n" after detecting the HELLO line.
     *   H5 replies with SYN-ACK so the GUI can confirm H5 acknowledges it. */
    if (strcmp(upper, "HILSYN") == 0)
    {
        static const char syn_ack[] = "[HIL] SYN-ACK — ground station acknowledged\r\n";
        HAL_UART_Transmit(&huart2, (uint8_t *)syn_ack, sizeof(syn_ack) - 1U, 20U);
        return;
    }

    /* ── HILGO — handshake step 3: confirm bidirectional link ────────────
     *   Ground station sends "HILGO\n" after receiving SYN-ACK.
     *   H5 replies with GO, confirming the link before injection begins. */
    if (strcmp(upper, "HILGO") == 0)
    {
        static const char go[] = "[HIL] GO — link verified, injection active\r\n";
        HAL_UART_Transmit(&huart2, (uint8_t *)go, sizeof(go) - 1U, 20U);
        return;
    }
#endif /* HIL_MODE */
...
void ESC_App_Init(void)
{
    ...
    SD_Logger_Init();
#ifndef HIL_MODE
    /* In HIL mode SD logging is not started automatically — SD card FAT
     * writes (100–200 ms) would starve the 10 Hz [HIL] status line. */
    SD_Logger_Start();
#endif
    Flight_Trigger_Init();
    Estimator_Init();
...
#ifdef HIL_MODE
    /* Announce readiness on USART2 so hil_gui.py can confirm the correct
     * firmware is flashed before the first [HIL] status line fires. */
    {
        static const char hil_hello[] = "[HIL] HELLO — H5 online, send HILSYN to handshake\r\n";
        HAL_UART_Transmit(&huart2, (uint8_t *)hil_hello, sizeof(hil_hello) - 1U, 100U);
    }
#endif
}
...
#ifndef HIL_MODE
    /* Camera SPI polling uses HAL_Delay() (up to 48 ms per call when MISO
     * is floating) and would cause USART2 RX FIFO overruns during HIL.
     * Battery ADC is also unused in HIL mode. */
    Camera_PollEchoTask();
